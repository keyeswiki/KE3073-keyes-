...
| 代码                   | 说明                                                         |
| ---------------------- | ------------------------------------------------------------ |
| pinMode(TouchPin, INPUT) | 由“ #define TouchPin  15 ”知道，定义电容触摸传感器模块的管脚为GPIO15，“INPUT”设置为输入模式。 |
| if( ){ } else{ }       | 如果（ ）里的表达式为真，则执行 if { }块内的代码。如果（ ）里表达式为假 ，则执行 else { }块内的代码。 |
| digitalRead(TouchPin)    | 这个函数是从指定的数字引脚TouchPin读取电容触摸传感器的数字电平，高(HIGH)或者低(LOW)。如果该引脚未连接任何东西，则digitalRead( )可以返回“高”或“低”（并且可以随机更改）。 |
...
| 代码                   | 说明                                     |
| ---------------------- | ---------------------------------------- |
| item = digitalRead(15) | 读取电容触摸传感器数字电平信号。 |
